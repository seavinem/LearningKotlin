...
data class ID(
  ...
  companion object {
    const val size = 10
    private val random = Random(47)
    private val source = listOf('a'..'z', 'A'..'Z', '0'..'9')
    private fun idGenerator() =
      (1..size).map { source.random(random) }
        .joinToString("")
...
class Bank(val name: String) {
  ...
  fun addID(id: ID) = applied.first { it.name == id.name }.addID(id)
